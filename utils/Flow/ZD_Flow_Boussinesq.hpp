...
**************************************************************************/
#ifndef _ZD_LIB_FLOW_BOUSSINESQ_HPP_
#define _ZD_LIB_FLOW_BOUSSINESQ_HPP_
...
namespace ZD {
    template <typename T>
    class CFlowBoussinesq : public CFlow<T, 2> {
    public:
    ...
    public:
        CFlowBoussinesq(const value_type minTime, const value_type maxTime, const char *scalarName = nullptr);
        ~CFlowBoussinesq();
...
template <typename T>
ZD::CFlowBoussinesq<T>::CFlowBoussinesq(const value_type minTime, const value_type maxTime, const char *scalarName)
{
...
template <typename T>
ZD::CFlowBoussinesq<T>::~CFlowBoussinesq()
{
...
template <typename T>
inline const bool ZD::CFlowBoussinesq<T>::CheckPosition(const point_type& p) const
{
...
template <typename T>
inline ZD::CIntegrator<T, 2> * ZD::CFlowBoussinesq<T>::CreateIntegrator() const
{
    return new CIntegratorRK4<T, 2>(this->m_stepSize);
}

template <typename T>
inline const ZD::CPoint<T, 2> ZD::CFlowBoussinesq<T>::Velocity(const point_type& p, const value_type& time) const
{
    int indices[2];
...
template <typename T>
inline const T ZD::CFlowBoussinesq<T>::Scalar(const point_type& p, const value_type& time) const
{
    int indices[2];
    TimeToIndex(time, indices);
    value_type f = (time - m_pTimes[indices[0]]) / (m_pTimes[indices[1]] - m_pTimes[indices[0]]);
...
template <typename T>
bool ZD::CFlowBoussinesq<T>::ReadTimeFile()
{
    ...
    if (fp == nullptr) {
        ...
        this->m_pTimes = new value_type[BOUSSINESQ_TOTAL_COUNT];
...
template <typename T>
bool ZD::CFlowBoussinesq<T>::ReadGerrisOutput(const value_type minTime, const value_type maxTime, const char *scalarName)
{
    assert(this->m_pDatasets == nullptr);

    this->m_pDatasets = new CFlowIOGerris<T>[BOUSSINESQ_TOTAL_COUNT];
    ...
    for (int i = minID; i <= maxID; ++i) {
        ...
        sprintf(pathname, "%ssim-%s.xml", BOUSSINESQ_DATA_PATH, timeStr.c_str());
...
template <typename T>
inline void ZD::CFlowBoussinesq<T>::TimeToIndex(const value_type time, int *indices) const
{
    indices[0] = 0;
...
template<typename T>
inline void ZD::CFlowBoussinesq<T>::GetBBox(point_type& min, point_type& max) const
{
