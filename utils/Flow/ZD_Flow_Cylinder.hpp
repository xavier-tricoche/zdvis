...
**************************************************************************/
#ifndef _ZD_LIB_FLOW_CYLINDER_HPP_
#define _ZD_LIB_FLOW_CYLINDER_HPP_
...
namespace ZD {
    template <typename T>
    class CFlowCylinder : public CFlow<T, 2> {
    public:
    ...
    public:
        CFlowCylinder(const value_type minTime, const value_type maxTime, const char *scalarName = nullptr);
        ~CFlowCylinder();
...
template <typename T>
ZD::CFlowCylinder<T>::CFlowCylinder(const value_type minTime, const value_type maxTime, const char *scalarName)
{
    ...
    m_maxTime = this->m_pTimes[CYLINDER_TOTAL_COUNT - 1];
...
template <typename T>
ZD::CFlowCylinder<T>::~CFlowCylinder()
{
...
template <typename T>
inline const bool ZD::CFlowCylinder<T>::CheckPosition(const point_type& p) const
{
...
template <typename T>
inline ZD::CIntegrator<T, 2> * ZD::CFlowCylinder<T>::CreateIntegrator() const
{
    return new CIntegratorRK4<T, 2>(this->m_stepSize);
}

template <typename T>
inline const ZD::CPoint<T, 2> ZD::CFlowCylinder<T>::Velocity(const point_type& p, const value_type& time) const
{
    int indices[2];
...
template <typename T>
inline const T ZD::CFlowCylinder<T>::Scalar(const point_type& p, const value_type& time) const
{
    int indices[2];
    TimeToIndex(time, indices);
    value_type f = (time - m_pTimes[indices[0]]) / (m_pTimes[indices[1]] - m_pTimes[indices[0]]);
...
template <typename T>
bool ZD::CFlowCylinder<T>::ReadTimeFile()
{
...
template <typename T>
bool ZD::CFlowCylinder<T>::ReadGerrisOutput(const value_type minTime, const value_type maxTime, const char *scalarName)
{
    assert(this->m_pDatasets == nullptr);

    this->m_pDatasets = new CFlowIOGerris<T>[CYLINDER_TOTAL_COUNT];
...
template <typename T>
inline void ZD::CFlowCylinder<T>::TimeToIndex(const value_type time, int *indices) const
{
    indices[0] = 0;
...
template<typename T>
inline void ZD::CFlowCylinder<T>::GetBBox(point_type& min, point_type& max) const
{
